...
def distribution_tiles_files_index(
    ...
    """
    path_quadtree_tsv = file_store.path.bag3d_export_dir(
        version=version.version,
    ).joinpath("quadtree.tsv")
    path_tiles_dir = file_store.path.bag3d_export_dir(
        version=version.version,
    ).joinpath("tiles")
...
def cityjsonfeatures_with_party_walls_nl(
    ...
    reconstructed_features_dir = file_store_fastssd.path.geoflow_crop_dir
