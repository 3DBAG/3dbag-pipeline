...
def mock_distribution_tiles_files_index(intermediate_data_dir)  -> TilesFilesIndex:
    return pickle.load(open(intermediate_data_dir / "distribution_tiles_files_index.pkl", "rb"))
...
def test_party_walls(context, intermediate_data_dir):
    ...
    result = party_walls_nl(context, mock_distribution_tiles_files_index(intermediate_data_dir))
