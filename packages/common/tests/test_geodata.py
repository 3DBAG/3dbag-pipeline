...
def test_pdal_info(context, test_data_dir):
    return_code, out_info = pdal_info(
        context.resources.pdal.app,
        Path(f"{test_data_dir}/2024_C_39GN2.LAZ"),
        with_all=False,
        verbose=False,
    )
    print(f"Return code: {return_code}")
    print(f"JSON output: {out_info}")
    #assert return_code != 0
    assert "Global encoding WKT flag" in str(out_info)
