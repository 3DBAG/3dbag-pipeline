...
def lasindex_ahn3(context, laz_files_ahn3):
    ...
    """
    cmd_list = ["{exe}", "-i {local_path}", "-append", "-tile_size",
                str(context.op_config["tile_size"])]
    if context.op_config["force"] is False:
        cmd_list.append("-dont_reindex")
    context.resources.lastools.execute("lasindex", " ".join(cmd_list),
                                       local_path=laz_files_ahn3.path)
...
def lasindex_ahn4(context, laz_files_ahn4):
    ...
    if context.op_config["force"] is False:
        cmd_list.append("-dont_reindex")
    context.resources.lastools.execute("lasindex", " ".join(cmd_list),
                                       local_path=laz_files_ahn4.path)
...
def regular_grid_200m(context):
    ...
    conn.send_query(
        SQL("COMMENT ON TABLE {table} IS 'Regular grid tile boundaries for partitioning LAZ files.'"),
        query_params={"table": new_table}
    )

    cellsize = 200
    bbox, nr_cells_x, nr_cells_y = generate_grid(bbox=PDOK_TILE_INDEX_BBOX,
                                                 cellsize=cellsize)
            ...
            for y_i in range(nr_cells_y):
                for x_i in range(nr_cells_x):
                    ...
                    query = inject_parameters(
                        SQL("INSERT INTO {table} (geom) VALUES(st_geomfromtext({wkt}))"),
    ...
    return Output(new_table, metadata={
        "nr_tiles": nr_cells_x * nr_cells_y, "celllsize": cellsize})
...
def laz_tiles_ahn3_200m(context, regular_grid_200m, metadata_table_ahn3):
    ...
    partition_laz_with_grid(context, metadata_table_ahn3, regular_grid_200m,
                            ahn_version=3, cellsize=200,
...
def laz_tiles_ahn4_200m(context, regular_grid_200m, metadata_table_ahn4):
    ...
    partition_laz_with_grid(context, metadata_table_ahn4, regular_grid_200m,
                            ahn_version=4, cellsize=200,
                            max_workers=context.op_config["max_workers"])


def partition_laz_with_grid(context, metadata_table_ahn, regular_grid_200m,
                            ahn_version, cellsize, max_workers):
    conn = context.resources.db_connection
    query_params = {
    ...
    out_dir = ahn_dir(context.resources.file_store.data_dir,
                      ahn_version=ahn_version).joinpath(f"tiles_{cellsize}m")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for tile, xmin, ymin, pdok_match in tile_ids:
            ...
            cmd = ['{exe}', '-v', '-i']
            cmd.extend(
                str(ahn_laz_dir(context.resources.file_store.data_dir,
                                ahn_version=ahn_version) / ahn_filename(t))
                for t in pdok_match
            )
            cmd += ['-inside_tile', str(xmin), str(ymin), str(tile_size), '-merged',
                    '-o', '{local_path}']
            future_to_tile[executor.submit(
                context.resources.lastools.execute,
                "las2las", " ".join(cmd), local_path=out_file, silent=True
            )] = tile
