...
2. **`ahn_tile_index`** — Download the AHN tile index (bladwijzer) and checksum files, and create the metadata tables for AHN 3, 4, 5 and 6. Must be run before the AHN download jobs.
3. **`ahn3`** / **`ahn4`** / **`ahn5`** / **`ahn6`** — Download the LAZ files (COPC.LAZ for AHN6) and record metadata for each AHN version. These jobs are partitioned by tile and can be run independently. The `ahn_checksum_sensor` automates triggering these jobs when PDOK updates the upstream checksum files.
...
6. **`reconstruct`** — Run the crop and reconstruct steps for the Netherlands (partitioned by tile).
7. **`export`** — Run the tyler export and 3D Tiles steps. To be run after the *floors_estimation* package has been run first — it includes the GeoPackage and 3D Tiles outputs.
8. **`deploy`** — Deploy the Netherlands data to the production servers.
9. **`release`** — Perform the final steps for the 3DBAG release (publish data and webservices).
